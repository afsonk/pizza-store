...
const singleItemSumSelector = createSelector(
    stateItemSelector,
    arr => arr.reduce((acc, next: CartItemType) => acc + next.price, 0)
)
...
const makeArray = createSelector(
    arrayMakingSelector,
    (arr: any): CartItemType[] => {
        const preArr = []
        for (let item in arr) {
...
export const {addItemToCart, removeItemFromCart, minusItemFromCart, clearCart, plusItemInCart} = cartSlice.actions