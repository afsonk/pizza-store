...
describe('PizzasList component test', () => {
  it('should render 3 contentItem components', () => {
    ...
    const store = configureStore({ reducer: { pizzaItems: pizzasSlice }, preloadedState })
    renderWithRedux(<PizzasList />, store)

    expect(screen.getByTestId('pizzasList')).toBeInTheDocument()
    expect(screen.getAllByTestId('contentItem')).toHaveLength(
      preloadedState.pizzaItems.pizzas.length
    )
  })

  it('should render 4 loading blocks when isLoading is true', () => {
    const preloadedState = {
      pizzaItems: {
        pizzas: [],
        isLoading: true
    ...
    expect(screen.getAllByTestId('loadingBlock')).toHaveLength(4)
  })
})
